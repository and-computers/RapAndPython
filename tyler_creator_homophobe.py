...
ref_df = create_references_df(songs_number_ref)
logger.info(ref_df)

grouped_ref_df = ref_df.groupby(['year'])
counted_vals = grouped_ref_df.count()



summed_ref_df = grouped_ref_df.sum()
summed_ref_df['year'] = summed_ref_df.index.astype(int)





traceHomophobe = go.Bar(x=summed_ref_df['year'],y=summed_ref_df['number of references'],
